...
import { useUserLogoutMutation } from '@store/reducers/userApiSlice';

interface Iprops {
  setViewMenu: (updateFn: (prev: boolean) => boolean) => void;
}

export default function Menu({ setViewMenu }: Iprops) {
  ...
  return (
    <div className={s.root} ref={ref}>
      <div className={s.header}>
        <p className={s.header__text}>{username}</p>
        <svg
          xmlns="http://www.w3.org/2000/svg"
          viewBox="0 0 24 24"
          className={s.exitMenu}
          onClick={() => {
      ...
      <div className={s.content}>
        <Search version={'Menu'} />
        <Position version={'Menu'} />

        {username ? (
          <>
            <Link to={'/orders'} className={s.orders__user}>
              <svg
                viewBox="0 0 1024 1024"
                fill="var(--var-text)"
                className={s.ordersIcon}
                version="1.1"
                xmlns="http://www.w3.org/2000/svg"
              >
                <path
            ...
            <Link to={'/account'} className={s.orders__user}>
              <svg
                version="1.0"
                xmlns="http://www.w3.org/2000/svg"
                width="50.000000pt"
              ...
              >
                ...
                >
                  <path
                    fill="var(--var-text)"
                    d="M219 449 c-15 -6 -32 -22 -38 -37 -15 -36 -9 -118 12 -166 29 -66 24
...
77 -33 24 -69 30 -101 18z"
      ...
      {username ? (
        <div
          className={s.signContainer}
          onClick={() => {
            toExit();
          }}
        >
          <BtnSecondary title={'Выйти'} />
        </div>
      ) : (
        <div className={s.signContainer}>
          <Link to={'/signIn'} className={s.sign}>
            <BtnPraimary title={'Войти'} />
          </Link>
          <Link to={'/signUp'} className={s.sign}>
            <BtnSecondary title={'Зарегистрироваться'} />
          </Link>
        </div>
      )}
    </div>
